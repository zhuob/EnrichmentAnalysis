...
#' @export
#'
plotPower <- function(folder,  background = "BACK0", showcol=1, textsize = rep(20, 4), xtext = c("DE percentage", "Power")){
  
  updated_case <- c("a", "b", "c", "d", "e")  ##  If I want to change the name of correlation structures
  
  r1 <- summarizePower(folder,  background, coln=showcol, case = "a" )
 
  
  case <- c("b", "c", "d", "e")
  for (i in 1:length(case))
  {
    r2 <- summarizePower(folder, background, coln=showcol, case = case[i] )
    r1 <- rbind(r1, r2)
  }
...
#' @export
#' 
createPowerTable <- function(folder, showcol, background = "BACK0", case = "a"){
