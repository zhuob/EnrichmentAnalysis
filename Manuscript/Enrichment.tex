...
\begin{document}
	...
	\subsection{Simulations}\label{subsection:simulation}
		...
		The simulation setup is as follows: first, we simulate an entire gene set containing $m=500$ genes, from which we sample $m_1 = 100$ genes to represent those in the test set, and the remaining $m_2=400$ genes those in the background set; second, for gene $i=1, \ldots, m$, we simulate the DE effect size $\delta_i$ from $N(2, 1)$ and DE indicators $Z_i$ from $\text{Binom}(1, p_i)$,  where $p_i= p_t$ if gene $i$ belongs to the test set and $p_i = p_b$  otherwise; third, we set the "true" mean expression values $\mu_1 = \bm 0_m$ and $\bm \mu_2 = \bm \Delta$, respectively,  for the control and treatment groups; fourth, $n_1$ samples are simulated from $\text{MVN}(\bm \mu_1, \bm \Sigma)$ for the control group and $n_2$ samples from $\text{MVN}(\bm \mu_2, \bm \Sigma)$ where the covariance $\bm \Sigma$ may represent one of the following cases: 
		\begin{enumerate}
			\item[(a0):] the genes are independent of each other.
			\item[(a):] only genes in the test set are correlated, with exchangeable correlation structure, that is, $\text{Cor}(Y_{i_1}, Y_{i_2})=\rho$ for $\forall i_1, i_2 \in I_t$.
			\item[(c):] all genes are correlated, with exchangeable correlation structure, that is, $\text{Cor}(Y_{i_1}, Y_{i_2})=\rho$ for $\forall i_1, i_2 \in I$.
			\item[(e):] genes are correlated within the test set and within the background set, but any two genes, one from each set, are independent. That is, the correlation structure is block diagonal, with $\text{Cor}(Y_{i_1}, Y_{i_2})=\rho_1$ for $i_1, i_2 \in I_t$  , $\text{Cor}(Y_{i_3}, Y_{i_4})=\rho_2$ for $i_3, i_4\in I_b$, and  $\text{Cor}(Y_{i_5}, Y_{i_6})= 0$ for $\forall ~i_5\in I_t, \forall~ i_6\in I_b$.
			\item[(f):] all genes are correlated, but the correlation between two genes depend on whether they belong to the test set or not. Specifically, $\text{Cor}(Y_{i_1}, Y_{i_2})=\rho_1$  for $i_1, i_2 \in I_t$,    $\text{Cor}(Y_{i_3}, Y_{i_4})=\rho_2$, for $ i_3, i_4\in I_b$, and  $\text{Cor}(Y_{i_5}, Y_{i_6})=\rho_3$ for $\forall~ i_5\in I_t, \forall~ i_6\in I_b$.
		...
		\subsubsection{Type I error simulations}
		  ...
		  Figure \ref{fig:typeIerror} shows the 
		...
		\begin{figure}[]
			\caption{Type I error rates for gene set tests, $p$-value distribution for case (a0) - (g) from left to right, from top to bottom, NO gene is DE}\label{fig:typeIerror}
			 \begin{center}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEA0.eps}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEA.eps}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEC.eps}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEE.eps}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEF.eps}
				\includegraphics[width=8cm,height=6cm]{Figures/NODEG.eps}
			\end{center} 
		\end{figure} 
