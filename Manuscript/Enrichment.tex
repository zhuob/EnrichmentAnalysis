...
\begin{document}
	...
	\textbf{Independent gene set test} \\
	 ...
	 as genes in a gene set are often correlated and function together.
	  ...
	  A number of methods have been proposed to accont for between-gene correlation in gene set tests. One attempt is to evaluate the set-level statistic by permuting the biological samples (see, for example, \cite{subramanian2005gene, efron2007testing}). Permuting samples does not require an explicit understanding of the underlying structure and thus protects the test against such correlation. The very famous example of this kind is the \textit{Gene Set Enrichment Analysis }(GSEA) proposed by \cite{subramanian2005gene}.  However, this approach implicitly alters the null hypothesis being tested and therefore it is diffucult to characterize the null and the alternative hypotheses \citep{goeman2007analyzing, khatri2012ten}. Another attempt is to conduct set-level test that works with the between-gene correlation structures. \cite{wu2012camera} proposed Correlation Adjusted MEan RAnk (CAMERA) gene set test that estimates a \textit{variance inflation factor} (VIF) associated with correaltions between gene expression profiles, and then incorporates it into the parametric or rank-based version of CAMERA test. \cite{yaari2013quantitative} also used the idea of incorporating VIF to adjust for correlation in their distribution-based gene set analysis. Valid estimation of VIF relies on the assumption that correlations between any two local statisitcs are almost the same as correlations between their corresponding expression profiles. This assumption has been demonstrated (??? a better word???) by \cite{barry2008statistical} for several gene-level statistics (e.g., $t$-statistic, Wald-type statistic for regressing expression on censored time-to-event data through a Cox proportional hazards model). However, as shown by (the paper to be finished), the correlation among gene-level $t$-test statistics can be badly estimated by sample correlations when a fraction of genes are DE. 
	  ...
	  % (This is a self-contained gene set test) \cite{huang2013gene} uses a multivariate linear regression model in which the between-gene correlations are explicitly modeled by a working covariance matrix. 
	
	\textbf{What do we propose?} \\
	In this paper, we propose a new competitive gene set test procedure that takes into account the correlations among gene-level statistics in the set-level test statistic. This procedure aims to correct for the discrepancy between correlation among expression profiles and that among gene-level statistics. Our stretagy is to model the covariance of gene-level statistics by two variance components, one attributable to the correlations among expression profiles and the other attributable to the DE effect associated with the treatment. OurMethod is a three-stage procedure: first, we use the mean difference between two experimental conditions as the gene-level statistics; then we estimate the covaraince from the data and incorporate it into the set-level statistic; lastly, we use the set-level statisitc to evaluate the significance of the test set. OurMethod works for a two group comparison experiment, and does not require any particular correlation structure. 
