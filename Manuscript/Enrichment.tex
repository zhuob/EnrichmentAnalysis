...
%\end{table}
		...
		\subsubsection{Power simulation}\label{subsection:typeIerror}		 
	...
	\begin{table}[ht]
		...
		Method & \multicolumn{4}{c}{DE size = 0.05} \\
			...
			\hline 
			OurTest & 0.660 & 0.967 & 0.998 & 1.000 \\ 
			lm & 0.674 & 0.970 & 0.997 & 1.000 \\ 
			ModeratedT & 0.856 & 0.994 & 0.999 & 1.000 \\ 
			MRSGE & 0.201 & 0.417 & 0.682 & 0.901 \\ 
			Camera & 0.663 & 0.964 & 0.997 & 1.000 \\ 
			CameraRank & 0.133 & 0.297 & 0.583 & 0.828 \\ 
			GSEA & 0.847 & 0.994 & 1.000 & 1.000 \\ 
			QUSAGE & 0.747 & 0.984 & 0.999 & 1.000 \\ 
			\hline
		\end{tabular}
	\end{table}
	...
	\begin{table}[ht]
		...
		Method & \multicolumn{4}{c}{DE size = 0.1} \\
			...
			\hline
		OurTest & 0.243 & 0.562 & 0.892 & 0.980 \\ 
		lm & 0.245 & 0.578 & 0.897 & 0.984 \\ 
		ModeratedT & 0.325 & 0.670 & 0.921 & 0.991 \\ 
		MRSGE & 0.176 & 0.413 & 0.716 & 0.898 \\ 
		Camera & 0.254 & 0.564 & 0.891 & 0.982 \\ 
		CameraRank & 0.130 & 0.272 & 0.620 & 0.810 \\ 
		GSEA & 0.000 & 0.000 & 0.000 & 0.000 \\ 
		QUSAGE & 0.265 & 0.589 & 0.899 & 0.987 \\ 
			\hline
		\end{tabular}
	\end{table}
	
		Figure \ref{fig:power} shows for OurMethod, the variations in power according to different correlation structures across four alternative scenarios. The left part is the power for group $A_3$ with a DE size of 0.05, and the right part is the power for group $A_4$ with a DE size of 0.1. For each correlation structure and each alternative, we calibrate the power at a significance level of 0.05 from 1000 simulated data sets. 
		The powers for case (a0) and (c) are very similar and among the highest under each of the four alternatives. It's not surprising because they correspond to the simplest correlation structures: in (a0) genes are simulated to be independent and in (c) genes are simulated to have the same correlation 0.1. As the correlation structure becomes more complicated, from (a) to (e) then to (f), the power decrease for every given alternative scenario. The power under correlation structure (f) is the lowest for both $A_3$ and $A_4$ simulations. We also note that the power decreases when DE genes are present in the background set as compared to the case where there are no DE genes in the background set. (We might need the same DE size to illustrate this point.)
		
			\begin{figure}[H]
				\caption{power: left, DE size = 0.05, and no DE genes in the background set; right; DE size = 0.10, DE probability is 0.1 in the background set.}\label{fig:power}
				\begin{center}
