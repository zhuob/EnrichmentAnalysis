...
\begin{document}
	...
	\textbf{What are the existing methods}\\
	 Most competitive gene set tests report a $p$-value from some form of global test that use local genewise statistics as observations. However, they fall into two groups: gene set tests that assume independence of genes, and those that doesn't. The former includes,  among many others, the fisher's exact test that examines the over-representation of DE genes in the test set (see \cite{huang2009bioinformatics} for a review), and permutation-based test that evaluates $p$-values by permuting gene lables (e.g., sigPathway of \cite{tian2005discovering}). However, tests assuming independence of genes may result in inflated false discovery rate (REF) and loss of power as genes in a gene set are often correlated and function together. For this reason, the tests allowing for between-gene correlations are...
	 
	
	\textbf{What are the problems with them} \\
