...
nperm                 = 999,             # Number of random permutations (default: 999)
...
# preproc.type control the type of pre-processing: threshold, variation filter, normalization
  ...
  class.labels <- trt
  gene.labels <- rownames(data)
  gene.set <- gene.labels[which(go_term == 1)]
  all.gene.descs <- gene.labels
  size.G <- length(gene.set)
...
#  p.vals <- matrix(0, nrow = Ng, ncol = 2)
  p.vals <- matrix(NA, nrow = Ng, ncol = 2)
  ...
  ## change made on Jan 1st, 2017, to avoid exactly zero p values.
  obj <- list(p.vals = (p.vals[1]*nperm + 1)/(nperm + 1), phi = phi, obs.phi = obs.phi, Obs.ES = Obs.ES)
